...
class FragmentWaiting : Fragment(), MyCallback.Companion.MyBookingCallback,
    SwipeRefreshLayout.OnRefreshListener, MyCallback.Companion.RefreshTokenCallback {

    companion object {
        @JvmStatic
        fun newInstance() = FragmentWaiting()
    }
    ...
    private fun setView(view : View){
        ...
        activity?.let {
            ...
            if (userSession.access_token != null){
                userController.getMyBooking("waiting",userSession.access_token ?: "",this)
            }
            else{
    ...
    override fun onMyBookingSuccess(data: ArrayList<HashMap<String, Any>>) {
        ...
        if (data.size > 0){
            ...
            myRecyclerView.adapter = TripsAdapter(fragmentActivity,data,2)
        }
        else{
